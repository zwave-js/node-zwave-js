...
async function maintenanceParse(): Promise<void> {
	...
	// Load the zwa files
	const zwaFiles = await enumFilesRecursive(processedDir, (file) =>
		file.endsWith(".json"),
	);
	for (const file of zwaFiles) {
		const j = await fs.readFile(file, "utf8");
		zwaData.push(JSON5.parse(j));
	}

	const searchString = "LED";
	const replaceString = "../templates/test_blah.json#test";
	let write = false;
	let oldParam = {};
	...
	for (const file of zwaData) {
		if (typeof file.paramInformation === "undefined") {
			continue;
		}

		if (typeof file.manufacturer === "undefined") {
			continue;
		}

		for (let [key, value] of Object.entries(file.paramInformation)) {
			if (value.label.includes(searchString)) {
				// Display the parameter
				await rl.write(`\nParameter: ${key}`);
				await rl.write(stringify(value, "\t"));

				await rl.question(
					"(r)eplace), (s)kip, (c)lose > ",
					function (line) {
						switch (line.trim()) {
							case "s":
								break;
							case "r":
								...
								// Add the import string
								value["$import"] = replaceString;
								write = true;
								break;
							case "c":
								rl.close();
				...
				await rl.write("New parameter:");
				await rl.write(stringify(value, "\t"));

				await rl.question("Save? (n) to cancel > ", function (line) {
					switch (line.trim()) {
						case "n":
							value = oldParam;
							break;
