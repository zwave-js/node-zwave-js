...
const transform: Transform = (
	...
	const ccImplementations = root.find(j.ClassDeclaration, {
		superClass(value) {
			return value?.type === "Identifier" && value.name.endsWith("CC");
		},
	});
	...
	const methods = ccImplementations.find(j.ClassMethod, {
		...
		return node.key.type === "Identifier"
			&& (node.key.name === "persistValues")
			&& node.params.length >= 1
			&& node.params[0].type === "Identifier"
			&& node.params[0].name.endsWith("applHost")
	...
	methods.replaceWith(({ node }) => {
		...
		// @ts-expect-error
		ident.typeAnnotation.typeAnnotation.typeParameters = undefined;
		return node;
	});

	const paramUsages = methods.find(j.Identifier, {
		name(value) {
			return value.endsWith("applHost");
		},
	});
	...
	targetDecl.at(0).get().node.specifiers.push(
		j.importSpecifier(j.identifier("PersistValuesContext")),
	);
