...
const transform: Transform = (
	...
	methods.replaceWith(({ node }) => {
		...
		ident.name = "ctx";
		// @ts-expect-error
		ident.typeAnnotation.typeAnnotation.typeName.name = "GetValueDB";
		return node;
	});

	const memberExprs = methods.find(j.MemberExpression, {
		object: {
			type: "Identifier",
			name: "applHost",
		},
	...
	memberExprs.replaceWith(({ node }) => {
		node.object = j.identifier("ctx");
		return node;
	});
