...
export type SerialAPICommandMachine2Input =
	| { value: "start" }
	| { value: "message sent" }
	| { value: "ACK" }
	| { value: "CAN" }
...
export type SerialAPICommandMachine2 = StateMachine<
	SerialAPICommandState,
	SerialAPICommandMachine2Input
>;
...
export function createSerialAPICommandMachine2(
	message: Message,
): SerialAPICommandMachine2 {
	const initialState: SerialAPICommandState = {
		value: "initial",
	};

	const transitions: InferStateMachineTransitions<SerialAPICommandMachine2> =
		(state) => (input) => {
			switch (state.value) {
				case "initial":
