...
const watcher = chokidar.watch(["build/**/*.js"], {
	cwd: process.cwd(),
	atomic: true,
});
...
async function exec(): Promise<void> {
	...
	for (const [filename, hash] of entries) {
		console.time("hashing " + filename);
		hasher.update(filename);
		hasher.update(hash);
		console.timeEnd("hashing " + filename);
	}
	...
	child.on("exit", (code) => {
		child = undefined;
		console.clear();
		console.log(gray(`Waiting for file changes... Press Ctrl+C to exit.`));
	});
}
