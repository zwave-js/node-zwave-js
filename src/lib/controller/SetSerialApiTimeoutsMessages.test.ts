...
describe("lib/controller/SetSerialApiTimeoutsRequest => ", () => {
	...
	it("with type Request", () => {
		...
		expect(getMessageTypeStatic(SetSerialApiTimeoutsRequest)).toBe(MessageType.Request);
	});
	it("and priority Controller", () => {
		expect(getDefaultPriority(req)).toBe(MessagePriority.Controller);
		expect(getDefaultPriorityStatic(SetSerialApiTimeoutsRequest)).toBe(MessagePriority.Controller);
	});
	it("and a function type SetSerialApiTimeouts", () => {
		expect(getFunctionType(req)).toBe(FunctionType.SetSerialApiTimeouts);
		expect(getFunctionTypeStatic(SetSerialApiTimeoutsRequest)).toBe(FunctionType.SetSerialApiTimeouts);
	});
	it("that expects a SetSerialApiTimeouts response", () => {
		expect(getExpectedResponse(req)).toBe(FunctionType.SetSerialApiTimeouts);
		expect(getExpectedResponseStatic(SetSerialApiTimeoutsRequest)).toBe(FunctionType.SetSerialApiTimeouts);
	});
...
describe("lib/controller/SetSerialApiTimeoutsResponse => ", () => {
	...
	it("with type Response", () => {
		...
		expect(getMessageTypeStatic(SetSerialApiTimeoutsResponse)).toBe(MessageType.Response);
	});
	it("and NO default priority", () => {
		expect(getDefaultPriority(res)).toBeUndefined();
		expect(getDefaultPriorityStatic(SetSerialApiTimeoutsResponse)).toBeUndefined();
	});
	it("and a function type SetSerialApiTimeouts", () => {
		expect(getFunctionType(res)).toBe(FunctionType.SetSerialApiTimeouts);
		expect(getFunctionTypeStatic(SetSerialApiTimeoutsResponse)).toBe(FunctionType.SetSerialApiTimeouts);
	});
	it("that expects NO response", () => {
		expect(getExpectedResponse(res)).toBeUndefined();
		expect(getExpectedResponseStatic(SetSerialApiTimeoutsResponse)).toBeUndefined();
	});
