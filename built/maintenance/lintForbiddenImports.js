...
exports.lintForbiddenImports = void 0;
...
const forbiddenImportsRegex = /^\/\* @forbiddenImports (?<forbidden>.*?) \*\/$/gm;
function lintForbiddenImports() {
        ...
        while (todo.length > 0) {
            ...
            for (const imp of imports) {
                switch (checkImport(imp)) {
                    ...
                    case "forbidden": {
                        numErrors++;
                        const message = `Found forbidden import of module ${imp.name}:
${[...importStack, `❌ ${imp.name}`]
    ...
    if (numErrors) {
        ...
        return Promise.reject(new Error("The forbiddenImports rule had an error! See log output for details."));
    }
    else {
        return Promise.resolve();
    }
}
exports.lintForbiddenImports = lintForbiddenImports;
if (require.main === module) {
    lintForbiddenImports()
        .then(() => process.exit(0))
        .catch(() => process.exit(1));
}
//# sourceMappingURL=lintForbiddenImports.js.map