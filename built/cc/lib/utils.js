...
exports.configureLifelineAssociations = exports.removeAssociations = exports.addAssociations = exports.getAllAssociationGroups = exports.getAssociationGroups = exports.isAssociationAllowed = exports.getAllAssociations = exports.getAssociations = void 0;
const safe_1 = require("@zwave-js/core/safe");
const safe_2 = require("@zwave-js/shared/safe");
const arrays_1 = require("alcalzone-shared/arrays");
const AssociationCC_1 = require("../cc/AssociationCC");
const AssociationGroupInfoCC_1 = require("../cc/AssociationGroupInfoCC");
const MultiChannelAssociationCC_1 = require("../cc/MultiChannelAssociationCC");
const API_1 = require("./API");
function getAssociations(applHost, endpoint) {
    const ret = new Map();
    if (endpoint.supportsCC(safe_1.CommandClasses.Association)) {
        const destinations = AssociationCC_1.AssociationCC.getAllDestinationsCached(applHost, endpoint);
    ...
    if (endpoint.supportsCC(safe_1.CommandClasses["Multi Channel Association"])) {
        const destinations = MultiChannelAssociationCC_1.MultiChannelAssociationCC.getAllDestinationsCached(applHost, endpoint);
        for (const [groupId, assocs] of destinations) {
            if (ret.has(groupId)) {
...
function getAllAssociations(applHost, node) {
...
function isAssociationAllowed(applHost, endpoint, group, destination) {
    ...
    if (destination.endpoint == undefined) {
        // "normal" association
        const sourceNodeCCVersion = endpoint.getCCVersion(safe_1.CommandClasses.Association);
        securityClassMustMatch = sourceNodeCCVersion < 3;
    }
    else {
        // multi channel association
        const sourceNodeCCVersion = endpoint.getCCVersion(safe_1.CommandClasses["Multi Channel Association"]);
    ...
    if (sourceSecurityClass != undefined && targetSecurityClass != undefined) {
        ...
        else if (!securityClassMustMatch &&
    ...
    if (!endpoint.supportsCC(safe_1.CommandClasses["Association Group Information"])) {
        return true;
    }
    // The following checks don't apply to Lifeline associations
    if (destination.nodeId === applHost.ownNodeId)
        return true;
    const groupCommandList = AssociationGroupInfoCC_1.AssociationGroupInfoCC.getIssuedCommandsCached(applHost, endpoint, group);
...
exports.isAssociationAllowed = isAssociationAllowed;
function getAssociationGroups(applHost, endpoint) {
        ...
        for (let group = 1; group <= groupCount; group++) {
            ...
            ret.set(group, {
                maxNodes: (multiChannel
                    ? mcInstance
                    : assocInstance).getMaxNodesCached(applHost, endpoint, group) || 1,
                ...
                assocConfig?.label ??
                    ...
                    `Unnamed group ${group}`,
                multiChannel,
                profile: agiInstance.getGroupProfileCached(applHost, endpoint, group),
                issuedCommands: agiInstance.getIssuedCommandsCached(applHost, endpoint, group),
            });
        }
    }
    else {
...
exports.getAssociationGroups = getAssociationGroups;
function getAllAssociationGroups(applHost, node) {
    const ret = new Map();
    for (const endpoint of node.getAllEndpoints()) {
        if (endpoint.supportsCC(safe_1.CommandClasses.Association)) {
...
exports.getAllAssociationGroups = getAllAssociationGroups;
async function addAssociations(applHost, endpoint, group, destinations) {
    ...
    const nodeAssociations = (0, arrays_1.distinct)(destinations
    ...
    if (endpoint.supportsCC(safe_1.CommandClasses.Association)) {
        ...
        throw new safe_1.ZWaveError(`Node ${nodeAndEndpointString} does not support associations!`, safe_1.ZWaveErrorCodes.CC_NotSupported);
    }
    if (endpoint.supportsCC(safe_1.CommandClasses["Multi Channel Association"])) {
        mcInstance = MultiChannelAssociationCC_1.MultiChannelAssociationCC;
    }
    else if (endpointAssociations.length > 0) {
        throw new safe_1.ZWaveError(`Node ${nodeAndEndpointString} does not support multi channel associations!`, safe_1.ZWaveErrorCodes.CC_NotSupported);
    }
    const assocGroupCount = assocInstance?.getGroupCountCached(applHost, endpoint) ?? 0;
    const mcGroupCount = mcInstance?.getGroupCountCached(applHost, endpoint) ?? 0;
    const groupCount = Math.max(assocGroupCount, mcGroupCount);
    if (group > groupCount) {
        throw new safe_1.ZWaveError(`Group ${group} does not exist on node ${nodeAndEndpointString}`, safe_1.ZWaveErrorCodes.AssociationCC_InvalidGroup);
    }
    const deviceConfig = applHost.getDeviceConfig?.(endpoint.nodeId);
    ...
    if (groupIsMultiChannel) {
        ...
        const disallowedAssociations = destinations.filter((a) => !isAssociationAllowed(applHost, endpoint, group, a));
        if (disallowedAssociations.length) {
            ...
            message += disallowedAssociations
                ...
                .join("");
            throw new safe_1.ZWaveError(message, safe_1.ZWaveErrorCodes.AssociationCC_NotAllowed);
        }
        // And add them
        const api = API_1.CCAPI.create(safe_1.CommandClasses["Multi Channel Association"], applHost, endpoint);
        ...
        if (destinations.some((a) => a.endpoint != undefined)) {
            throw new safe_1.ZWaveError(`Node ${nodeAndEndpointString}, group ${group} does not support multi channel associations!`, safe_1.ZWaveErrorCodes.CC_NotSupported);
        }
        // Check that all associations are allowed
        const disallowedAssociations = destinations.filter((a) => !isAssociationAllowed(applHost, endpoint, group, a));
        if (disallowedAssociations.length) {
            throw new safe_1.ZWaveError(`The associations to the following nodes are not allowed: ${disallowedAssociations
                .map((a) => a.nodeId)
                .join(", ")}`, safe_1.ZWaveErrorCodes.AssociationCC_NotAllowed);
        }
...
exports.addAssociations = addAssociations;
async function removeAssociations(applHost, endpoint, group, destinations) {
    const nodeAndEndpointString = `${endpoint.nodeId}${endpoint.index > 0 ? `, endpoint ${endpoint.index}` : ""}`;
    // Split associations into conventional and endpoint associations
    const nodeAssociations = (0, arrays_1.distinct)(destinations
        .filter((a) => a.endpoint == undefined)
    ...
    if (!groupExistsAsMultiChannel && !groupExistsAsNodeAssociation) {
        throw new safe_1.ZWaveError(` Association group ${group} does not exist for node ${nodeAndEndpointString}`, safe_1.ZWaveErrorCodes.AssociationCC_InvalidGroup);
    }
    if (endpointAssociations.length > 0 && !groupExistsAsMultiChannel) {
        throw new safe_1.ZWaveError(`Node ${nodeAndEndpointString}, association group ${group} does not support multi channel associations!`, safe_1.ZWaveErrorCodes.AssociationCC_InvalidGroup);
    }
...
async function configureLifelineAssociations(applHost, endpoint) {
    ...
    const nodeSupportsMultiChannel = node.supportsCC(safe_1.CommandClasses["Multi Channel"]);
    let assocInstance;
    const assocAPI = API_1.CCAPI.create(safe_1.CommandClasses.Association, applHost, endpoint);
    if (endpoint.supportsCC(safe_1.CommandClasses.Association)) {
        assocInstance = AssociationCC_1.AssociationCC;
    }
    let mcInstance;
    ...
    if (lifelineGroups.length === 0) {
        ...
        valueDB.setValue(AssociationCC_1.AssociationCCValues.hasLifeline.endpoint(endpoint.index), false);
        return;
    }
    applHost.controllerLog.logNode(node.id, {
        endpoint: endpoint.index,
...
must use endpoint association: ${mustUseMultiChannelAssociation}`,
        ...
        const invalidEndpointAssociations = mcInstance
            ?.getAllDestinationsCached(applHost, endpoint)
            .get(group)
...
must use node association:     ${rootMustUseNodeAssociation}`,
            ...
            if (!rootMustUseNodeAssociation) {
                const rootNodesValueId = MultiChannelAssociationCC_1.MultiChannelAssociationCCValues.nodeIds(group).id;
                const rootHasNodeAssociation = !!valueDB
                    .getValue(rootNodesValueId)
                    ?.some((a) => a === ownNodeId);
                const rootEndpointsValueId = MultiChannelAssociationCC_1.MultiChannelAssociationCCValues.endpoints(group).id;
                const rootHasEndpointAssociation = !!valueDB
                    .getValue(rootEndpointsValueId)
                ...
                if (rootHasEndpointAssociation) {
                    ...
                    const rootMCAPI = API_1.CCAPI.create(safe_1.CommandClasses["Multi Channel Association"], applHost, node);
    ...
    valueDB.setValue(AssociationCC_1.AssociationCCValues.hasLifeline.endpoint(endpoint.index), true);
}
exports.configureLifelineAssociations = configureLifelineAssociations;
